...
export interface SafeAuthAdapter<TAdapter> {
  ...
  subscribe(
    event: SafeAuthEvent<TAdapter>,
    handler: SafeAuthEventListener<TAdapter>
  ): void;
  unsubscribe(
    event: SafeAuthEvent<TAdapter>,
    handler: SafeAuthEventListener<TAdapter>
...
export interface ISafeAuthKit<TAdapter> {
  ...
  subscribe(
    event: SafeAuthEvent<TAdapter>,
    listener: SafeAuthEventListener<TAdapter>
  ): void;
  unsubscribe(
    event: SafeAuthEvent<TAdapter>,
    listener: SafeAuthEventListener<TAdapter>
...
export type SafeAuthEventListener<T> = T extends Web3AuthAdapter
