...
test('test', async ({ page }) => {
  await page.evaluate((data) => {
    (window as any).ethereum = data;
  }, 'test');
  await page.goto('/en');
  await new Promise((resolve) => setTimeout(resolve, 7000));
  await expect(
    page.getByRole('button', { name: 'alpha_user@test.io' }),
  ).toBeVisible();
  ...
  await page.locator('button:has-text("Select passes")').first().click();
  await new Promise((resolve) => setTimeout(resolve, 7000));
  await page
    .locator('div')
  ...
  await new Promise((resolve) => setTimeout(resolve, 7000));
  await page.getByRole('button', { name: 'An event An event 1 pass' }).click();
  await expect(
