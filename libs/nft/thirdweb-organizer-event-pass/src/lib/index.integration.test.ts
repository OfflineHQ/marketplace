...
import { CollectionDeploymentError, EventPassCollection } from './index';
...
describe('EventPassCollection', () => {
  let eventPassCollection: EventPassCollection;
  let client: PgClient;
  ...
  describe('deployEventPassCollection - happy path', () => {
    const mockDeployer = {
    ...
    it('should successfully deploy a NFT drop collection', async () => {
      eventPassCollection = new EventPassCollection(mockedThirdwebSDKInstance);
      let event = (
      ...
      expect(alchemyInfos).toEqual({
        ...
        activityWebhookSigningKey: 'mocked_signing_key',
        metadataUpdateWebhookId: null,
        metadataUpdateWebhookSigningKey: null,
      });
  ...
  describe('deployEventPassCollection - errors', () => {
    beforeAll(() => {
    ...
    it('should throw an error if deployAnEventPassNftDropCollection fail', async () => {
      const event = (
        await adminSdk.GetEventWithPassesOrganizer({
          slug: eventPassEventSlug,
          locale: 'en' as Locale,
      ...
      await expect(
        ...
        new CollectionDeploymentError(
          new Error('Error deploying a normal collection : Error in lazyMint'),
        ),
      );
    });
    it('should throw an error if deployEventPassDelayedRevealCollection fail', async () => {
      const event = (
        await adminSdk.GetEventWithPassesOrganizer({
          slug: eventPassEventSlug,
          locale: 'en' as Locale,
      ...
      await expect(
        ...
        new CollectionDeploymentError(
          new Error(
            'Error deploying a delayed reveal collection : Error in createDelayedRevealBatch',
          ),
        ),
      );
    });
  ...
  describe('revealEventPassDelayedContract', () => {
    let eventPassCollection: EventPassCollection;
    ...
    it('should update the status of isDelayedReveal and return the list of currentOwnerAddress', async () => {
      const owners =
        await eventPassCollection.revealEventPassDelayedContract(
          contractAddress,
        );
    ...
    it('should throw an error when reveal fails', async () => {
      ...
      await expect(
        eventPassCollection.revealEventPassDelayedContract(contractAddress),
