...
BEGIN
  ...
  IF (TG_OP = 'UPDATE' AND TG_LEVEL = 'ROW') THEN
    ...
    SELECT
      jsonb_object_agg(new_t.key, new_t.value) - excluded_cols INTO audit_row.changed_fields
    FROM
      jsonb_each(old_r) AS old_t
  JOIN jsonb_each(new_r) AS new_t ON (old_t.key = new_t.key
      AND old_t.value <> new_t.value);
  ELSIF (TG_OP = 'DELETE'
...
CREATE OR REPLACE FUNCTION audit.audit_table(target_table regclass, audit_rows boolean, audit_query_text boolean)
  ...
  AS $body$
  SELECT
    audit.audit_table($1, $2, $3, ARRAY[]::text[]);
$body$
LANGUAGE SQL;
...
CREATE OR REPLACE FUNCTION audit.audit_table(target_table regclass)
  ...
  AS $body$
  SELECT
    audit.audit_table($1, BOOLEAN 't', BOOLEAN 't');
$body$
LANGUAGE 'sql';
